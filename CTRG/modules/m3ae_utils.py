...
from ..gadgets.my_metrics import Accuracy, Scalar, VQARADScore, BLUE
    ...
    for split in ["train", "val", "test"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                setattr(pl_module, f"{split}_{k}_accuracy", Scalar())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
                ...
                setattr(pl_module, f"{split}_{k}_BLEU_2", BLUE())
                setattr(pl_module, f"{split}_{k}_BLEU_1", Scalar())
                '''
                setattr(pl_module, f"{split}_{k}_BLEU_2", BLUE())
                setattr(pl_module, f"{split}_{k}_METEOR", BLUE())
                setattr(pl_module, f"{split}_{k}_ROUGE_L", BLUE())
                '''
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            '''
            setattr(pl_module, f"{split}_{k}_BLEU_1", compute_scores())
            setattr(pl_module, f"{split}_{k}_BLEU_2", compute_scores())
            setattr(pl_module, f"{split}_{k}_METEOR", compute_scores())
            setattr(pl_module, f"{split}_{k}_ROUGE_L", compute_scores())
            setattr(pl_module, f"{split}_{k}_loss", Scalar())
            '''
        
        elif loss_name == "rg":
            value = getattr(pl_module, f"{phase}_{loss_name}_BLEU_1").compute()
            pl_module.log(f"{loss_name}/{phase}/BLEU_1", value)
            getattr(pl_module, f"{phase}_{loss_name}_BLEU_2").reset()
            getattr(pl_module, f"{phase}_{loss_name}_BLEU_1").reset()
            pl_module.log(
                f"{loss_name}/{phase}/loss_epoch",
                getattr(pl_module, f"{phase}_{loss_name}_loss").compute(),
            )
            # value = getattr(pl_module, f"{phase}_{loss_name}_loss").compute().item()
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
        else:
            raise ValueError

        print(value, getattr(pl_module, f"{phase}_{loss_name}_loss").compute().item())
        the_metric += value
